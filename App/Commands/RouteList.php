...
class RouteList extends Command
{
        ...
        foreach (Router::getInstance()->getRoutes() as $method => $routes) {
            ...
            foreach ($routes as $uri => $configuration) {
                $formatedRoutes[] = [
                    ...
                    'is_fallback' => $configuration->isFallback() ?
                        (new StringWrapper('true'))->foreground(ConsoleStyleColor::BrightGreen)->wrap() :
                        (new StringWrapper('false'))->foreground(ConsoleStyleColor::BrightRed)->wrap()
                ];
            }
        }
    ...
    private function getTableHeader(): array
    {
        return [
            ...
            (new StringWrapper('IS FALLBACK'))->foreground(ConsoleStyleColor::BrightCyan)->wrap(),
        ];
    }
}
